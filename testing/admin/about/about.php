...
if(isset($_POST["save"])) {
    ...
    if($type && ($type['mime'] != 'image/png' || $type['mime'] !='image/jpg' || $type['mime'] = 'image/jpeg')){ if($_FILES['im']['name'] <1024 * 1000) { $upload='..img/'.$_FILES['im']['name'];
    }
        if (move_uploaded_file($_FILES['im']['tmp_name'], $upload)) echo "Файл загружен"; else echo "Ошибка при загрузке файла";
    }
